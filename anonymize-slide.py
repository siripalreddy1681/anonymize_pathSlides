...
# Note: Only run on copies of svs file, as this application directly modifies the provided file path

#!/usr/bin/env python3
#
#  anonymize-slide_rmvmacro.py - Delete the slide and macro label from a whole-slide image
#
#  Original copyright:
#    (c) 2007-2013 Carnegie Mellon University
#    (c) 2011      Google, Inc.
#    (c) 2014      Benjamin Gilbert
#  Python 3 port (non-affiliated) by ChatGPT for user convenience.
#
...
class TiffFile(FileIO):
    def __init__(self, path: str):
        # open read/write binary like Python2's file('r+b')
        ...
        while True:
            ...
            if not self.directories and not self._bigtiff:
                # NDPI detection (after reading first IFD)
    ...
    def fmt_size(self, fmt: str) -> int:
        return struct.calcsize(self._convert_format(fmt))

    def near_pointer(self, base: int, offset: int) -> int:
        # NDPI: return the value whose low-order 32-bits are equal to @offset
        # and which is within 4 GB of @base and below it. Otherwise, return offset.
        if self._ndpi and offset < base:
            seg_size = 1 << 32
            offset += ((base - offset) // seg_size) * seg_size
        return offset

    def read_fmt(self, fmt: str, force_list: bool = False):
        fmt2 = self._convert_format(fmt)
        vals = struct.unpack(fmt2, self.read(struct.calcsize(fmt2)))
    ...
    def write_fmt(self, fmt: str, *args):
        fmt2 = self._convert_format(fmt)
        self.write(struct.pack(fmt2, *args))


class TiffDirectory(object):
    def __init__(self, fh: TiffFile, number: int, in_pointer_offset: int):
        self.entries = {}
        count = fh.read_fmt('Y')
    ...
    def delete(self, expected_prefix: bytes = None):
        ...
        # Remove directory (link around it)
        if DEBUG:
...
class TiffEntry(object):
    ...
    def value(self):
        ...
        if self.type == ASCII:
            ...
            return b''.join(items[:-1]).decode(errors='replace')
        else:
            return items
...
class MrxsFile(object):
    def __init__(self, filename: str):
        ...
        self._dat = configparser.RawConfigParser()
        self._dat.optionxform = str  # preserve case
        try:
            with open(self._slidedatfile, 'rb') as fh:
                ...
                self._dat.read_file(StringIO(data_str))
        except IOError:
            raise UnrecognizedFile
        ...
        file_count = self._dat.getint('DATAFILE', 'FILE_COUNT')
        self._datafiles = [
    ...
    def _set_key(self, section, key, value):
        ...
        self._dat.set(section, key, str(value))
...
def accept(filename, formatname):
    if DEBUG:
        print(f"{filename}: {formatname}")


def do_aperio_svs(filename):
    with TiffFile(filename) as fh:
        # Looser SVS handling:
        # if it's TIFF-readable, scan all directories for label/macro entries
        accept(filename, 'SVS')

         # Find and delete label and macro images
        found = False
        ...
        if not found:
            raise IOError("No label or macro image in SVS file")
